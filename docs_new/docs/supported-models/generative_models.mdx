...
<tr>
      ...
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}>Pure-recurrent (attention-free) RWKV-7 language models with O(1) per-sequence state and no KV cache. Supports FLA-format and native <code>rwkv-rs/hf-adapter</code> checkpoints, dynamic batching, chunked prefill, TP/PP, decode CUDA graphs, and state-aware radix prefix caching with the no-buffer Mamba strategy. Use <code>--trust-remote-code</code>; prefill CUDA graphs are disabled for this architecture. A compatibility path is available for NVIDIA SM70 by using FP16, the PyTorch sampling backend, and disabled CUDA graphs.</td>
    </tr>
<tr>
