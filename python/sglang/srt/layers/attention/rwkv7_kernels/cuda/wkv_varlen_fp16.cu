...
#pragma unroll
  for (int pair = 0; pair < kPairs; ++pair) {
    state_registers[pair] = __halves2half2(
        state_shared[pair * 2][column],
  ...
  for (int token = begin; token < end; ++token) {
    ...
    if (column < kPairs) {
      ...
      decay_shared[column] = __halves2half2(
          __float2half_rn(expf(__half2float(w[pair_base]))),
      ...
      neg_kk_shared[column] =
          __halves2half2(__hneg(kk_even), __hneg(kk_odd));
      b_shared[column] = __halves2half2(
          __hmul(kk_even, a[pair_base]),
...
#pragma unroll
    for (int pair = 0; pair < kPairs; ++pair) {
      sa_pair = __hfma2(
          neg_kk_shared[pair], state_registers[pair], sa_pair);
    }
...
#pragma unroll
    for (int pair = 0; pair < kPairs; ++pair) {
      half2 updated = __hfma2(
          state_registers[pair],
          decay_shared[pair],
          __hfma2(
              b_shared[pair],
              sa2,
              __hmul2(k_shared[pair], value2)));
      state_registers[pair] = updated;
      y_pair = __hfma2(updated, r_shared[pair], y_pair);
    }
    output[base + column] =
        __hadd(__low2half(y_pair), __high2half(y_pair));
    __syncthreads();
  }
...
void rwkv7_wkv_varlen_fp16_cuda(
  ...
  TORCH_CHECK(r.scalar_type() == at::kHalf && w.scalar_type() == at::kHalf &&
                  k.scalar_type() == at::kHalf && v.scalar_type() == at::kHalf &&
  ...
  TORCH_CHECK(r.is_contiguous() && w.is_contiguous() && k.is_contiguous() &&
                  v.is_contiguous() && kk.is_contiguous() && a.is_contiguous() &&
  ...
  TORCH_CHECK(
      r.device() == state_pool.device() && w.device() == state_pool.device() &&
          k.device() == state_pool.device() && v.device() == state_pool.device() &&
          kk.device() == state_pool.device() && a.device() == state_pool.device() &&
